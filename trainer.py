...
class Trainer(object):
            ...
            while iterator < len(self.data_loader):
                ...
                while d_iter < d_iter_count and iterator < len(self.data_loader):
                    ...
                    noise = Variable(torch.randn(right_images.size(
                        0), self.noise_dim), volatile=True).cuda()
    ...
    def _train_gan(self, cls):
        ...
        for epoch in range(self.num_epochs):
            for sample in self.data_loader:
                iteration += 1
                right_images = sample['right_images'].float()  # .cuda()
                right_embed = sample['right_embed'].float()  # .cuda()
                wrong_images = sample['wrong_images'].float()  # .cuda()

                real_labels = torch.ones(right_images.size(0))  # .cuda()
    ...
    def predict(self):
        for sample in self.data_loader:
            ...
            right_embed = right_embed.float()  # .cuda()

            # Train the generator
            noise = torch.randn(right_images.size(0), 100)  # .cuda()
            noise = noise.view(noise.size(0), 100, 1, 1)
            fake_images = self.generator(right_embed, noise)
            ...
            for image, t in zip(fake_images, txt):
                ...
                im.save(
                    'results/{0}/{1}.jpg'.format(self.save_path, t.replace("/", "")[:100]))
                print(t)
